<template>
  ...
  >
    ...
    >
      ...
      <div v-if="account.platform === 'anthropic'" class="space-y-4 rounded-lg border border-gray-200 p-4 dark:border-dark-600">
        <div>
          <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('admin.customization.accountTitle') }}</h3>
          <p class="input-hint">{{ t('admin.customization.accountHint') }}</p>
        </div>
        <div class="grid gap-4 sm:grid-cols-2">
          <label>
            <span class="input-label">{{ t('admin.customization.fields.fallback_policy.label') }}</span>
            <select v-model="claudeOverrides.fallback_policy" class="input mt-1 w-full">
              <option :value="null">{{ t('admin.customization.inherit') }}</option>
              <option value="native_passthrough">{{ t('admin.customization.options.native_passthrough') }}</option>
          ...
          <label>
            ...
            <select v-model="claudeOverrides.beta_policy_mode" class="input mt-1 w-full">
              <option :value="null">{{ t('admin.customization.inherit') }}</option>
              <option value="capability_aware">{{ t('admin.customization.options.capability_aware') }}</option>
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <label v-for="key in claudeBooleanOverrideKeys" :key="key">
            <span class="input-label">{{ t(`admin.customization.fields.${key}.label`) }}</span>
            <select v-model="claudeOverrides[key]" class="input mt-1 w-full">
              <option :value="null">{{ t('admin.customization.inherit') }}</option>
              <option :value="true">{{ t('common.enabled') }}</option>
              <option :value="false">{{ t('common.disabled') }}</option>
            </select>
            <span class="mt-0.5 block text-xs text-gray-500">{{ t(`admin.customization.fields.${key}.hint`) }}</span>
          </label>
        </div>
      </div>
...
const claudeBooleanOverrideKeys = [
  'thinking_prefilter_enabled',
  'thinking_signature_retry_enabled',
  'thinking_tool_downgrade_retry_enabled',
  'fingerprint_unification',
  'metadata_passthrough',
  'url_redaction_enabled'
] as const
const claudeOverrides = reactive<Record<string, boolean | string | null>>({
  ...
  fingerprint_unification: null,
  metadata_passthrough: null,
  url_redaction_enabled: null
})

const loadClaudeCustomization = async (accountID: number) => {
  if (props.account?.platform !== 'anthropic') return
  try {
    const response = await adminAPI.accounts.getClaudeCustomization(accountID)
    for (const key of Object.keys(claudeOverrides)) {
      claudeOverrides[key] = Object.prototype.hasOwnProperty.call(response.overrides, key)
        ? (response.overrides[key] as boolean | string | null)
        : null
    }
  } catch {
    // Account editing remains usable if the optional compatibility endpoint is unavailable.
  }
}

const persistClaudeCustomization = async (accountID: number) => {
  if (props.account?.platform !== 'anthropic') return
  await adminAPI.accounts.updateClaudeCustomization(accountID, { ...claudeOverrides })
}
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  void loadClaudeCustomization(newAccount.id)
...
const submitUpdateAccount = async (accountID: number, updatePayload: Record<string, unknown>) => {
  ...
  try {
    ...
    await persistClaudeCustomization(accountID)
