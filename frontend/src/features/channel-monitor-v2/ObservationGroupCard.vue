  ...
  <article class="card flex h-full min-w-0 flex-col !rounded-lg p-5" :data-group-id="item.group_id">
    ...
    <div class="mt-auto">
      <div class="mb-2 flex items-center justify-between gap-2 text-xs text-gray-400">
        ...
        <button type="button" class="inline-flex items-center gap-1 text-primary-600 dark:text-primary-400" @click="$emit('detail', item)">{{ t('channelMonitorV2.observation.models', { count: item.models.length }) }}<Icon name="chevronRight" size="xs" /></button>
      </div>
