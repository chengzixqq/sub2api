<template>
  <div class="flex flex-wrap gap-1.5">
    <span v-if="coverageState !== 'complete'" class="badge badge-warning">{{ t(`channelMonitorV2.observation.coverage.${coverageState}`) }}</span>
    ...
    <template v-else-if="coverageState !== 'unavailable'">
...
const { t } = useI18n()
// Coverage and sample sufficiency are independent dimensions. A partial or
// delayed source must not hide a critical group state, and must not be shown
// as if it were the sample state itself.
const state = computed(() => props.metrics.sample_state)
const coverageState = computed(() => props.coverage.state)
</script>
