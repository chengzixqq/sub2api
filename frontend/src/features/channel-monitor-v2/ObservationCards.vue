  ...
  <section class="space-y-8">
    ...
    <template v-else>
      ...
      <section v-for="section in sections" :key="section.platform" class="space-y-3">
        ...
        <div v-if="layout === 'cards'" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4"><ObservationGroupCard v-for="item in section.items" :key="item.group_id" :item="item" :coverage="overview.coverage" :admin="admin" @detail="emit('detail', $event)" @bucket="handleBucket" /></div>
...
const emit = defineEmits<{
  toggleLayout: []
  detail: [item: ObservationChannel]
  bucket: [item: ObservationChannel, slot: { start: string; bucket: ObservationBucket | null }]
  retry: []
}>()
const { t } = useI18n()
...
function handleBucket(item: ObservationChannel, slot: { start: string; bucket: ObservationBucket | null }) {
  emit('bucket', item, slot)
}
</script>
