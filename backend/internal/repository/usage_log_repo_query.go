...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		billingProvenance         sql.NullString
		...
		createdAt                 time.Time
		probeCoalesced            bool
		probeLeaderRequestID      sql.NullString
	...
	if billingProvenance.Valid {
		log.BillingProvenance = &billingProvenance.String
	}
	...
	if probeLeaderRequestID.Valid {
		log.ProbeLeaderRequestID = &probeLeaderRequestID.String
	}
