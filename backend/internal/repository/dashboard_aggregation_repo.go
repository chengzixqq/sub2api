	...
	query := `
		WITH hourly AS (
			SELECT
				...
				COALESCE(SUM(CASE WHEN provider_cost_recorded THEN COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1) ELSE 0 END), 0) AS account_cost,
