...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.GET("/:id/customization", h.Admin.Account.GetClaudeCustomization)
		accounts.PUT("/:id/customization", h.Admin.Account.UpdateClaudeCustomization)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		// Claude 魔改兼容策略（全局预设）
		adminSettings.GET("/customization", h.Admin.Setting.GetClaudeCustomization)
		adminSettings.PUT("/customization", h.Admin.Setting.UpdateClaudeCustomization)
		adminSettings.POST("/customization/apply-preset", h.Admin.Setting.ApplyClaudeCustomizationPreset)
		adminSettings.POST("/customization/reset", h.Admin.Setting.ResetClaudeCustomization)
		// Web Search 模拟配置
...
func registerChannelMonitorV2Routes(admin *gin.RouterGroup, h *handler.Handlers, settingService *service.SettingService) {
	...
	monitor := admin.Group("/channel-monitor-v2")
	{
		...
		config.Use(featureGuard)
		{
			...
			config.GET("/observation-config", h.ChannelMonitorV2.ObservationConfig)
			config.PUT("/observation-config", h.ChannelMonitorV2.UpdateObservationConfig)
		}
		reads := monitor.Group("")
		reads.Use(modeV2Guard)
		{
			reads.GET("/dimensions", h.ChannelMonitorV2.Dimensions)
			reads.GET("/overview", h.ChannelMonitorV2.AdminObservationOverview)
