...
func maxReasoningEffortBillingMultiplierForChannel(model, effort string, pricing *ChannelModelPricing) float64 {
	if NormalizeMaxReasoningEffort(effort) == "max" && pricing != nil && pricing.MaxReasoningEffortMultiplier != nil && *pricing.MaxReasoningEffortMultiplier > 0 {
		return *pricing.MaxReasoningEffortMultiplier
	}
	return maxReasoningEffortBillingMultiplier(model, effort, nil)
}
