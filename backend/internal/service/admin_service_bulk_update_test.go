...
// 归属过滤的语义由专门的作用域测试覆盖。
func (s *accountRepoStubForBulkUpdate) GetByIDScoped(ctx context.Context, id int64) (*Account, error) {
...
// vendorBulkCtx 构造权限档全开的 vendor 作用域。
//
// 权限故意全开：本组断言的是「按归属剔除 ID」，
// 不能让结论侥幸依赖某个权限档恰好关闭。
func vendorBulkCtx(workspaceID int64) context.Context {
...
// TestBulkUpdateAccountsDropsForeignIDs 锁定别家账号 ID 在落库前被剔除。
//
// 批量端点只吃 account_ids，绕过了单条 GetByIDScoped 的归属校验。
// 不剔除的话，vendor 传一串猜出来的 ID 就能批量改写别家账号。
func TestBulkUpdateAccountsDropsForeignIDs(t *testing.T) {
	repo := &accountRepoStubForBulkUpdate{
		getByIDsAccounts: []*Account{
			{ID: 7, WorkspaceID: 3},  // 自家
			{ID: 11, WorkspaceID: 9}, // 别家
			{ID: 12, WorkspaceID: 3}, // 自家
		},
	}
	svc := &adminServiceImpl{accountRepo: repo}

	result, err := svc.BulkUpdateAccounts(vendorBulkCtx(3), &BulkUpdateAccountsInput{
		AccountIDs: []int64{7, 11, 12},
		Status:     StatusActive,
	})
	require.NoError(t, err)

	// 关键断言：传给仓储的 ID 里不含别家的 11。
	require.Equal(t, []int64{7, 12}, repo.bulkUpdateIDs)
	require.Equal(t, 2, result.Success)
}

// TestBulkUpdateAccountsAdminSkipsScopeQuery 锁定站长视角逐字不变。
//
// 工作区机制的硬约束：admin 必须与引入之前完全一致。
// 因此不查归属（getByIDsCalled 为 false），ID 原样落库。
func TestBulkUpdateAccountsAdminSkipsScopeQuery(t *testing.T) {
	repo := &accountRepoStubForBulkUpdate{}
	svc := &adminServiceImpl{accountRepo: repo}

	_, err := svc.BulkUpdateAccounts(WithScope(context.Background(), AdminScope()), &BulkUpdateAccountsInput{
		AccountIDs: []int64{7, 11},
		Status:     StatusActive,
	})
	require.NoError(t, err)
	require.False(t, repo.getByIDsCalled, "站长视角不应查询归属")
	require.Equal(t, []int64{7, 11}, repo.bulkUpdateIDs)
}

// TestBulkUpdateAccountsScopeQueryErrorBlocksWrite 锁定归属查询失败不降级为放行。
//
// 与计费路径的取舍相反：计费在请求已转发之后，查询失败必须回退以免错账；
// 这里是写入前的准入判定，失败只能拒绝，绝不能带着未校验的 ID 落库。
func TestBulkUpdateAccountsScopeQueryErrorBlocksWrite(t *testing.T) {
	boom := errors.New("账号表不可达")
	repo := &accountRepoStubForBulkUpdate{getByIDsErr: boom}
	svc := &adminServiceImpl{accountRepo: repo}

	_, err := svc.BulkUpdateAccounts(vendorBulkCtx(3), &BulkUpdateAccountsInput{
		AccountIDs: []int64{7},
		Status:     StatusActive,
	})
	require.Error(t, err)
	require.Nil(t, repo.bulkUpdateIDs, "查询失败时不得落库")
}
