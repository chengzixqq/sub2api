...
func (s *adminServiceImpl) ListGroups(ctx context.Context, page, pageSize int, platform, status, search string, isExclusive *bool, sortBy, sortOrder string) ([]Group, int64, error) {
	...
	groups, result, err := s.listGroupRows(ctx, params, platform, status, search, isExclusive)
	if err != nil {
		return nil, 0, err
	}
...
func (s *adminServiceImpl) listGroupRows(ctx context.Context, params pagination.PaginationParams, platform, status, search string, isExclusive *bool) ([]Group, *pagination.PaginationResult, error) {
	if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple {
		...
		if !ok {
			return nil, nil, errors.New("group repository does not support simple-mode filtering")
		}
		return repo.ListBindableWithFilters(ctx, params, platform, status, search, isExclusive)
	}
